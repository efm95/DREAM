    ...
    def __init__(self,
        ...
        for i in range(len(activity_types)):
            ...
            if activity == 'wave':
                ...
                self.activity_list.append(self.wave_function(samp))
            else:
    ...
    def generate(self):
        ...
        for event in range(self.n_events):
            ...
            for pos in range(3,self.n_exo_stat+3):
                self.event_log[event,pos] = self.activity_list[pos-3][sender]
                self.non_event_log[event,pos] = self.activity_list[pos-3][sender]
            
            # receiver statistics
            for pos in range(3*2,self.n_exo_stat+(3*2)):
                self.event_log[event,pos] = self.activity_list[pos-(3*2)][receiver]
                self.non_event_log[event,pos] = self.activity_list[pos-(3*2)][receiver]
